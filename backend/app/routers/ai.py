...
def _local_match_payload(needs, all_vols, candidate_vols, busy_vols, db: Session, source="local-fallback"):
    ...
    return {
        "source": source,
        "model": GROQ_MODEL if source == "groq" else "local-fallback",
        "matches": matches,
        "unmatched": unmatched,
        "kpis": {
        ...
        "alerts": [
            *([{"id": "ai-alert-groq", "type": "warning", "message": "Groq AI is unavailable or quota-limited, so safe local scoring is active.", "timestamp": "just now"}] if source != "groq" else []),
...
def _call_groq(needs, volunteers, active_counts):
    ...
    system_msg = (
        "You are an AI disaster-response dispatcher. "
        "Match unassigned community needs to non-offline volunteers using only the supplied live data. "
        "Do not invent volunteers, needs, locations, distances, or scores. Return ONLY valid JSON, no markdown. "
...
async def get_suggestions(db: Session = Depends(get_db), current_user: UserSchema = Depends(get_current_user)):
    ...
    groq_result = _call_groq(needs, candidate_vols, active_counts)
    if not groq_result:
    ...
    result = {
        **fallback,
        "source": "groq",
        "model": GROQ_MODEL,
        "matches": groq_result.get("matches", fallback["matches"]),
        "unmatched": groq_result.get("unmatched", fallback["unmatched"]),
        "alerts": groq_result.get("alerts", fallback["alerts"]),
        "activities": groq_result.get("activities", fallback["activities"]),
    }
...
async def assign_match(payload: dict, db: Session = Depends(get_db), current_user: UserSchema = Depends(get_current_user)):
    ...
    vol_user = db.query(User).filter(User.id == volunteer.user_id).first() if volunteer.user_id else None
